...
def whale(
    ...
    whale = accounts.add()
    eth_whale.transfer(whale, "40_000 ether")
    
    if token.symbol() == 'WETH':
        token.deposit({"value": "40_000 ether", "from": whale})
        assert token.balanceOf(whale) == Wei("40_000 ether")
    else:
