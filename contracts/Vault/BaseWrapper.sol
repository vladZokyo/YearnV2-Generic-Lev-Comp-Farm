...
abstract contract BaseWrapper {
    ...
    function setRegistry(address _registry) external {
        require(msg.sender == registry.governance(), "Only governance can change the registry!");
        // In case you want to override the registry instead of re-deploying
        registry = RegistryAPI(_registry);
        // Make sure there's no change in governance
        // NOTE: Also avoid bricking the wrapper from setting a bad registry
        require(msg.sender == registry.governance(), "In new registry governance must be equal to old registry governance!");
    }
    ...
    function _migrate(
        ...
        if (_amount > 0) {
            ...
            require(withdrawn.sub(migrated) <= maxMigrationLoss, "Migration losses are more than possible!");
